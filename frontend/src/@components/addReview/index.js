...
import { productReviewsLoggedinUser } from '../../@actions/productActions/productReviewLoggedinUser'
let initialValues = {
    comment: '',
}
...
const AddReview = ({ productId }) => {
    ...
    const {loading, error, reviewsLoggedinUser} = useSelector(state => state.reviewsLoggedinUser)
...
const handleRatingsChange = (newRating) => {
    ...
    const addReview = async ({ comment }) => {
        ...
        await dispatch(productReviewsLoggedinUser(productId, comment, rating))
                    ...
                    <Col lg='6' md='8' sm='10'>
                        <Card>
                            <Card.Body>
                                ...
                                {reviewsLoggedinUser && <div className = 'text-center text-success mt-3'>{reviewsLoggedinUser.message}</div>}
                            </Card.Body>
                        </Card>
                    </Col>
                </Row>
