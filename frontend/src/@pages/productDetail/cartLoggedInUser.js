...
import CartContainer2 from '../../@components/loggedinUserCart'
const CartLoggedInUser = ({ prod }) => {
    const [cartCon, showCartCon] = useState(false)
    const [quantity, setQuantity] = useState(1)
    const dispatch = useDispatch()
    const { addToCartLoggedinUser, addToCartLoggedinUserError, addToCartLoggedinUserLoading } = useSelector(state => state.addToCartLoggedinUser)
    ...
    const addToCart = () => {
        ...
        console.log('add to cart ', addToCartLoggedinUser)
    }
    if (addToCartLoggedinUserError) {
        alert('Item cannot be added to the cart')
        return (
            <>
                {addToCartLoggedinUserLoading ? <div></div> : <Row style={{ marginLeft: '0.5vw' }} className='main'>
                    <Col>
        ...
        return (
            <>
                {addToCartLoggedinUserLoading ? <div></div> : <Row style={{ marginLeft: '0.5vw' }} className='main'>
                    ...
                    {cartCon && <CartContainer2 onCloseCart={onCloseCart} msg={addToCartLoggedinUser.message} />}
                </Row>}
            </>
        )
    }
...
export default CartLoggedInUser